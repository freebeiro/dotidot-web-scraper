...
RSpec.describe 'Foundation Services Integration' do
  let(:url_validator) { Services::UrlValidatorService.new }
  let(:http_client) { Services::HttpClientService.new }
  let(:html_parser) { Services::HtmlParserService.new }
  let(:css_extractor) { Services::CssExtractionStrategy.new }
  ...
  describe 'end-to-end scraping workflow' do
    ...
    it 'successfully scrapes and extracts data from a valid URL' do
      # Step 1: Validate URL
      validation_result = url_validator.validate(target_url)
      ...
      fetch_result = http_client.fetch(validation_result[:url])
      expect(fetch_result[:success]).to be true
      expect(fetch_result[:body]).to eq(html_content)

      # Step 3: Parse HTML
      parse_result = html_parser.parse(fetch_result[:body])
      expect(parse_result[:success]).to be true
      expect(parse_result[:doc]).to be_a(Nokogiri::HTML::Document)

      # Step 4: Extract data
      fields = [
        Services::ExtractedField.new(name: 'page_title', selector: 'title', type: 'text'),
        Services::ExtractedField.new(name: 'site_title', selector: 'h1.site-title', type: 'text'),
        Services::ExtractedField.new(name: 'product_names', selector: '.product-name', type: 'text', multiple: true),
        Services::ExtractedField.new(name: 'product_prices', selector: '.price', type: 'text', multiple: true),
        Services::ExtractedField.new(name: 'product_ids', selector: '.product', type: 'attribute', attribute: 'data-id', multiple: true),
        Services::ExtractedField.new(name: 'stock_counts', selector: '.stock .count', type: 'text', multiple: true)
      ]

      extract_result = css_extractor.extract(parse_result[:doc], fields)
      ...
      expect(data['product_ids']).to eq(['123', '456'])
      expect(data['stock_counts']).to eq(['5', '25'])
    end
    ...
    it 'handles redirects in the workflow' do
      ...
      fetch_result = http_client.fetch(target_url)
      expect(fetch_result[:success]).to be true
      expect(fetch_result[:url]).to eq(redirect_url)

      # Continue with parsing and extraction
      parse_result = html_parser.parse(fetch_result[:body])
      expect(parse_result[:success]).to be true

      field = Services::ExtractedField.new(name: 'title', selector: 'title', type: 'text')
      extract_result = css_extractor.extract(parse_result[:doc], [field])
    ...
    it 'gracefully handles errors at each stage' do
      ...
      validation_result = url_validator.validate(invalid_url)
      ...
      field = Services::ExtractedField.new(name: 'bad', selector: '!!!invalid', type: 'text')
      extract_result = css_extractor.extract(doc, [field])
    ...
    describe 'complex extraction scenarios' do
      ...
      it 'extracts nested and normalized data' do
        ...
        # Full pipeline
        validation = url_validator.validate(target_url)
        fetch = http_client.fetch(validation[:url])
        parse = html_parser.parse(fetch[:body])
        
        fields = [
          Services::ExtractedField.new(
            name: 'item_titles', 
            selector: '.item h3', 
            type: 'text',
            multiple: true
        ...
        extract = css_extractor.extract(parse[:doc], fields)
        ...
        expect(extract[:data]['all_tags']).to eq(['Tag1', 'Tag2', 'Tag3'])
    ...
    describe 'performance and edge cases' do
      ...
      it 'handles malformed HTML with recovery' do
        ...
        fields = [
          Services::ExtractedField.new(name: 'title', selector: 'h1', type: 'text'),
          Services::ExtractedField.new(name: 'text', selector: '.inner span', type: 'text')
        ]
      ...
      it 'handles encoding issues across the pipeline' do
        ...
        fields = [
          Services::ExtractedField.new(name: 'title', selector: 'h1', type: 'text'),
          Services::ExtractedField.new(name: 'emoji', selector: '.emoji', type: 'text'),
          Services::ExtractedField.new(name: 'quotes', selector: '.special', type: 'text')
        ]
